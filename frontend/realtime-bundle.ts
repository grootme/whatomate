/**
 * Whatomate Real-Time Bundle
 * 
 * Monitoreo → Análisis → Decisión en tiempo real para WhatsApp
 * 
 * Flujo:
 *   1. MONITOREO: Poll de mensajes del Baileys bridge (solo texto)
 *   2. ANÁLISIS: Extracción de entidades, detección de patrones, sentimiento
 *   3. DECISIÓN: Generación automática de decisiones cuando se detectan patrones críticos
 * 
 * Todo se almacena en el Cognitive API (SQLite + FTS5)
 * y se expone vía REST + SSE para el frontend.
 */
...
const stmtInsertProcessed = db.prepare(`
  INSERT OR IGNORE INTO processed_messages 
    (message_id, chat_id, sender_id, sender_name, body, is_group, timestamp, 
     sentiment_score, sentiment_label, urgency, entities_extracted, decisions_generated)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`)

const stmtInsertEvent = db.prepare(`
  INSERT INTO bundle_events (event_type, source, severity, title, description, data)
  VALUES (?, ?, ?, ?, ?, ?)
`)

const stmtInsertEntityMention = db.prepare(`
  INSERT INTO entity_mentions (entity_name, entity_type, message_id, chat_id, context)
  VALUES (?, ?, ?, ?, ?)
`)
...
const stmtInsertDecision = db.prepare(`
  INSERT INTO auto_decisions (title, description, context, trigger_pattern, trigger_entity, status, priority, confidence)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?)
`)

// ─── Text Analysis Engine (local, no LLM needed for v1) ─────────────────────

// Sentiment keywords (Spanish + English)
...
function analyzeText(text: string): AnalysisResult {
  ...
  // Named entity heuristic (capitalized words that aren't sentence starts)
  ...
  // Topic extraction (significant words)
  ...
  // Add top keywords as topic entities
...
async function storeInCognitiveAPI(message: any, analysis: AnalysisResult) {
  try {
    // Store message
      ...
      try {
        await fetch(`${COGNITIVE_URL}/entities`, {
          ...
          body: JSON.stringify({
            name: entity.name,
            type: entity.type,
            attributes: { context: entity.context, source: 'whatsapp_auto' },
          }),
        })
      } catch {}
    }

    // If action items or decision keywords detected, create a decision
...
function processMessage(msg: any): ProcessedMessage | null {
  ...
  // Store entities
  for (const entity of analysis.entities) {
    stmtInsertEntityMention.run(entity.name, entity.type, messageId, chatId, entity.context)
  ...
  for (const trigger of DECISION_TRIGGERS) {
    if (trigger.check(stats)) {
      // Avoid duplicate decisions for the same trigger within 5 minutes
      ...
      if (!recentDuplicate) {
        ...
        // Log event
        stmtInsertEvent.run('decision', 'whatsapp', trigger.priority, trigger.name, trigger.description, JSON.stringify({ messageId, chatId, senderId }))
  ...
  storeInCognitiveAPI({ chatId, senderId, senderName, body, isGroup }, analysis)
...
async function pollBridgeMessages() {
  try {
    ...
    // Bridge might not be connected yet
...
function startMonitoring() {
  ...
  stmtInsertEvent.run('monitoring_started', 'system', 'info', 'Monitoreo Iniciado', 'El bundle de monitoreo en tiempo real ha iniciado', '{}')

  pollTimer = setInterval(pollBridgeMessages, POLL_INTERVAL_MS)
  console.log(`[bundle] Monitoring started — polling every ${POLL_INTERVAL_MS}ms`)
}
...
function stopMonitoring() {
  ...
  stmtInsertEvent.run('monitoring_stopped', 'system', 'info', 'Monitoreo Detenido', 'El bundle de monitoreo ha sido detenido', '{}')
...
app.post('/monitoring/start', (req, res) => {
  startMonitoring()
  res.json({ status: 'monitoring', message: 'Monitoreo en tiempo real iniciado' })
})
...
// Analysis endpoint — analyze text without storing
...
app.get('/stream', (req, res) => {
  ...
  // Send initial state
  res.write(`event: connected\ndata: ${JSON.stringify({ isMonitoring, totalProcessed })}\n\n`)
...
app.listen(BUNDLE_PORT, '0.0.0.0', () => {
  console.log(`[bundle] Whatomate Real-Time Bundle running on http://0.0.0.0:${BUNDLE_PORT}`)
