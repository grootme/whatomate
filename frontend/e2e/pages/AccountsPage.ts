export class AccountsPage extends BasePage {
  ...
  readonly whatsmeowClientTypeRadio: Locator
  ...
  constructor(page: Page) {
    ...
    this.whatsmeowClientTypeRadio = this.dialog.locator('label[for="whatsmeow"]')
    this.qrCodeDisplay = this.page.locator('.rounded-lg.bg-muted.p-4.text-center')
    this.startSessionButton = this.dialog.getByRole('button', { name: /Start Session/i })
    this.qrCodeImage = this.qrCodeDisplay.locator('canvas')
  }
  ...
  async selectWhatsmeowClientType() {
    await this.whatsmeowClientTypeRadio.click()
  }

  async startWhatsmeowSession(accountName: string) {
    const card = this.getAccountCard(accountName)
    await card.getByRole('button', { name: /Start Session/i }).click()
  }

  async expectQrCodeVisible(accountName: string) {
    const card = this.getAccountCard(accountName)
    await expect(card.locator('canvas')).toBeVisible()
  }

  async expectQrCodeHidden(accountName: string) {
    const card = this.getAccountCard(accountName)
    await expect(card.locator('canvas')).not.toBeVisible()
  }
  ...
  async fillAccountForm(options: {
    ...
    accessToken?: string
    verifyToken?: string
    clientType?: 'meta' | 'whatsmeow'
  }) {
    await this.dialog.locator('input#name').fill(options.name)

    if (options.clientType === 'whatsmeow') {
      await this.selectWhatsmeowClientType()
      await this.dialog.locator('input#phone_id').fill(options.phoneId)
    } else {
      // Default to Meta if not specified or 'meta'
      await this.dialog.locator('input#phone_id').fill(options.phoneId)
