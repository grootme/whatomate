...
let lastQR = null; // Store last QR string for /qr endpoint
...
// Also allow any host that looks like a local/private address
function isLocalHost(hostOnly) {
  if (_ACCEPTED_HOST_VALUES.has(hostOnly)) return true;
  // Allow private IP ranges (10.x, 172.16-31.x, 192.168.x)
  if (/^(10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.)/.test(hostOnly)) return true;
  // Allow *.space-z.ai preview domains for agent proxy
  if (hostOnly.endsWith('.space-z.ai') || hostOnly.endsWith('.space.chatglm.site')) return true;
  return false;
}
...
app.use((req, res, next) => {
  ...
  if (!isLocalHost(hostOnly)) {
    return res.status(400).json({
      error: 'Invalid Host header. Bridge accepts loopback/private hosts only.',
    });
  }
  next();
});
...
// QR code endpoint — returns PNG image or base64 data URL
app.get('/qr', async (req, res) => {
  if (connectionState === 'connected') {
    return res.json({ status: 'connected', qr: null, message: 'WhatsApp already connected' });
  }
  if (!lastQR) {
    return res.json({ status: connectionState, qr: null, message: 'QR code not yet generated. Wait a moment...' });
  }
  try {
    // Try to use the 'qrcode' npm package for PNG generation
    ...
    // Default: return JSON with base64 data URL
    res.json({ status: connectionState, qr: dataUrl, message: 'Scan with WhatsApp' });
  } catch (err) {
    // Fallback: return raw QR string
    res.json({ status: connectionState, qr: lastQR, message: 'QR code available (raw string)', raw: true });
  }
});
...
if (PAIR_ONLY) {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`🌉 WhatsApp bridge listening on 0.0.0.0:${PORT} (mode: ${WHATSAPP_MODE})`);
