...
func (toc *TelegramOSINTCorrelatorV2) findTemporalCorrelations(messages []RawMessage, osintData *OSINTSnapshot) []TemporalCorrelation {
        ...
        for _, msg := range messages {
                ...
                // Check maritime/ship data with temporal proximity for messages containing maritime keywords
                contentLower := strings.ToLower(msg.Content)
                maritimeKeywords := []string{"ship", "vessel", "port", "cargo", "tanker", "naval",
                        "maritime", "strait", "gulf", "fleet", "warship", "piracy", "blockade", "convoy"}
                hasMaritimeTerm := false
                for _, kw := range maritimeKeywords {
                        if strings.Contains(contentLower, kw) {
                                hasMaritimeTerm = true
                                break
                        }
                }

                if hasMaritimeTerm {
                        for _, ship := range osintData.Ships {
                                shipTypeLower := strings.ToLower(ship.Type)
                                confidence := 0.35
                                // Boost confidence for military vessels or named ship types
                                if shipTypeLower == "warship" || shipTypeLower == "naval" || shipTypeLower == "military" {
                                        confidence = 0.5
                                }
                                // Check if the ship name or type is explicitly mentioned
                                if ship.Name != "" && strings.Contains(contentLower, strings.ToLower(ship.Name)) {
                                        confidence = 0.75
                                } else if ship.Type != "" && strings.Contains(contentLower, shipTypeLower) {
                                        confidence = 0.6
                                }
                                if confidence > 0.35 {
                                        correlations = append(correlations, TemporalCorrelation{
                                                MessageID:     msg.ID,
                                                OSINTCategory: "maritime",
                                                OSINTEventID:  uuid.New().String(),
                                                TimeDelta:     0, // Ship data is not timestamped individually
                                                Confidence:    confidence,
                                                Details:       fmt.Sprintf("Maritime message correlates with vessel %s [%s] at %.4f,%.4f (speed: %.1f kn)", ship.Name, ship.Type, ship.Lat, ship.Lon, ship.Speed),
                                                Timestamp:     time.Now(),
                                        })
                                }
                        }
...
func correlateMessageContent(msg RawMessage, osintData *OSINTSnapshot) []CorrelationMatch {
        ...
        // Maritime/ship keyword list for maritime-specific correlation
        maritimeKeywords := []string{"ship", "vessel", "port", "cargo", "tanker", "naval",
                "maritime", "strait", "gulf", "fleet", "warship", "destroyer", "frigate",
                "carrier", "submarine", "ferry", "tugboat", "anchorage", "harbor", "harbour",
                "dock", "pier", "wharf", "quay", "berth", "channel", "waterway", "canal",
                "piracy", "pirate", "embargo", "sanctions", "blockade", "convoy", "escort"}

        // Maritime keyword detection: check if the message mentions any maritime terms
        hasMaritimeKeywords := false
        for _, kw := range maritimeKeywords {
                if strings.Contains(contentLower, kw) {
                        hasMaritimeKeywords = true
                        break
                }
        }

        // Correlate with maritime/ship data when maritime keywords are detected
        if hasMaritimeKeywords {
                for _, ship := range osintData.Ships {
                        shipNameLower := strings.ToLower(ship.Name)
                        shipTypeLower := strings.ToLower(ship.Type)
                        confidence := 0.55

                        // Higher confidence for direct ship name mentions
                        if shipNameLower != "" && strings.Contains(contentLower, shipNameLower) {
                                confidence = 0.85
                        } else if shipTypeLower != "" && strings.Contains(contentLower, shipTypeLower) {
                                confidence = 0.7
                        } else if shipTypeLower == "warship" || shipTypeLower == "naval" || shipTypeLower == "military" {
                                // Military vessel near a maritime keyword bump
                                confidence = 0.75
                        } else {
                                // General maritime keyword match — correlate with nearby vessel
                                continue
                        }
