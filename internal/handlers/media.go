...
// DownloadAndSaveMedia downloads media from WhatsApp and saves it locally
// Returns the local file path (relative to media storage) or error
func (a *App) DownloadAndSaveMedia(ctx context.Context, mediaID string, mimeType string, account *models.WhatsAppAccount) (string, error) {
        // Get the media URL
...
// DownloadAndSaveWhatsmeowMedia downloads media from a whatsmeow message event and saves it locally.
// DownloadMediaCustom downloads media from a whatsmeow message event
func (a *App) DownloadMediaCustom(ctx context.Context, account *models.WhatsAppAccount, evt *events.Message) ([]byte, error) {
        return a.WhatsApp.DownloadMediaCustom(ctx, account, evt)
}

// DownloadAndSaveWhatsmeowMedia downloads media from a whatsmeow message event and saves it locally.
func (a *App) DownloadAndSaveWhatsmeowMedia(ctx context.Context, account *models.WhatsAppAccount, evt *events.Message) (string, string, error) {
        // Determine mime type and filename from the whatsmeow event
        var mimeType, filename string
        protoMsg := evt.Message

        if img := protoMsg.GetImageMessage(); img != nil {
                mimeType = img.GetMimetype()
                filename = fmt.Sprintf("img_%s", evt.Info.ID) + getExtensionFromMimeType(mimeType)
        } else if vid := protoMsg.GetVideoMessage(); vid != nil {
                mimeType = vid.GetMimetype()
                filename = fmt.Sprintf("vid_%s", evt.Info.ID) + getExtensionFromMimeType(mimeType)
        } else if aud := protoMsg.GetAudioMessage(); aud != nil {
                mimeType = aud.GetMimetype()
                filename = fmt.Sprintf("aud_%s", evt.Info.ID) + getExtensionFromMimeType(mimeType)
        } else if doc := protoMsg.GetDocumentMessage(); doc != nil {
                mimeType = doc.GetMimetype()
                filename = doc.GetFileName()
                if filename == "" {
                        filename = fmt.Sprintf("doc_%s", evt.Info.ID) + getExtensionFromMimeType(mimeType)
                }
        } else if stk := protoMsg.GetStickerMessage(); stk != nil {
                mimeType = stk.GetMimetype()
                filename = fmt.Sprintf("sticker_%s", evt.Info.ID) + getExtensionFromMimeType(mimeType)
        } else {
                return "", "", fmt.Errorf("message does not contain identifiable media for download")
        }

        if mimeType == "" {
                return "", "", fmt.Errorf("could not determine mime type for media")
        }

        // Call the whatsmeow adapter's custom download method
        data, err := a.DownloadMediaCustom(ctx, account, evt)
        if err != nil {
                return "", "", fmt.Errorf("failed to download media from whatsmeow: %w", err)
        }

        // Determine subdirectory based on media type
        var subdir string
        switch {
        case strings.HasPrefix(mimeType, "image/") || strings.HasPrefix(mimeType, "sticker/"):
                subdir = "images"
        case strings.HasPrefix(mimeType, "video/"):
                subdir = "videos"
        case strings.HasPrefix(mimeType, "audio/"):
        ...
        // Return relative path and actual mime type for storage in database
        relativePath := filepath.Join(subdir, filename)
        a.Log.Info("Whatsmeow media saved", "path", relativePath, "size", len(data), "mime_type", mimeType)
