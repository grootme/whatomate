...
func TestMultiClientProvider_GetAdapter_Meta(t *testing.T) {
	log := logf.New(logf.Opt{Level: logf.DebugLevel})
...
func TestMultiClientProvider_SetIncomingMessageHandler_SetsBoth(t *testing.T) {
	...
	// Verify both adapters have the handler set
	assert.NotNil(t, provider.metaAdapter.incomingMessageHandler, "Meta adapter should have handler set")
	assert.NotNil(t, provider.whatsmeowAdapter.incomingMessageHandler, "Whatsmeow adapter should have handler set")
}
...
func TestMultiClientProvider_UnsupportedMethods_Whatsmeow(t *testing.T) {
	...
	t.Run("GetAnalytics returns not supported", func(t *testing.T) {
		_, err := provider.GetAnalytics(context.Background(), account, AnalyticsTypeMessage, nil)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "not supported by whatsmeow")
	})
}
