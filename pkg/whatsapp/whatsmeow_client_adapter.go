...
func (w *WhatsmeowClientAdapter) GetClient(account *models.WhatsAppAccount) (*whatsmeow.Client, error) {
	...
	if account.PhoneID == "" {
		return nil, fmt.Errorf("phone_id (JID) is required for whatsmeow client initialization")
	}

	jid, err := types.ParseJID(account.PhoneID)
	if err != nil {
		return nil, fmt.Errorf("invalid phone_id (JID): %w", err)
	}
...
func (w *WhatsmeowClientAdapter) eventHandler(client *whatsmeow.Client, account *models.WhatsAppAccount) func(evt interface{}) {
	return func(evt interface{}) {
		...
		case *events.QrCode:
			if v.Event == "code" {
				w.log.Info("Received whatsmeow QR code.", "account", account.Name)
				w.db.Model(&models.WhatsAppAccount{}).Where("id = ?", account.ID).Update("qr_code", v.Code)
				if w.wshub != nil {
					w.wshub.BroadcastToOrg(account.OrganizationID, websocket.WSMessage{
						Type: "whatsapp_qr_code",
						Payload: map[string]interface{}{
		...
		case *events.PairStatus:
			w.log.Info("Received whatsmeow pairing status.", "account", account.Name, "status", v.Status)

		case *events.LoggedIn:
			w.log.Info("Whatsmeow client logged in successfully.", "account", account.Name, "JID", v.JID)
			w.db.Model(&models.WhatsAppAccount{}).Where("id = ?", account.ID).Updates(map[string]interface{}{
				"status":   "active",
				"qr_code":   "",
				"phone_id": v.JID.String(),
			})
