...
class WhatomateClient {
  ...
  async authenticate(): Promise<void> {
    if (WHATOMATE_API_KEY) return; // API key auth, no login needed
  ...
  async request(method: string, path: string, data?: any, params?: any): Promise<any> {
    try {
  ...
  get(path: string, params?: any) { return this.request("GET", path, undefined, params); }
...
server.tool("send_text_message", "Send a text message to a WhatsApp contact", {
  ...
  const payload: any = { content, message_type: "text" };
...
server.tool("health_check", "Check Whatomate API server health status", {}, async () => {
  try {
    ...
    return { content: [{ type: "text" as const, text: `Health check failed: ${error.message}` }] };
  }
});
...
server.tool("ready_check", "Check if Whatomate API is ready (database + Redis)", {}, async () => {
  try {
    ...
    return { content: [{ type: "text" as const, text: `Ready check failed: ${error.message}` }] };
  }
});
