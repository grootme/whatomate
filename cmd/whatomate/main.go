...
func runServer(args []string) {
        ...
        // Connect active whatsmeow clients on startup
        app.ConnectWhatsmeowClients(context.Background())
...
func setupRoutes(g *fastglue.Fastglue, app *handlers.App, lo logf.Logger, basePath string) {
        ...
        g.POST("/api/accounts/{id}/start-session", app.StartWhatsAppSession)
        ...
        g.GET("/api/intel/health/aggregated", app.GetIntelAggregatedHealth)
        g.GET("/api/intel/threat-level/osint", app.GetIntelOSINTThreatLevel)
        g.GET("/api/intel/correlations", app.GetIntelCorrelations)
        g.POST("/api/intel/reports/schedule", app.ScheduleIntelReport)
        g.GET("/api/intel/reports/scheduled", app.ListScheduledReports)
        g.DELETE("/api/intel/reports/scheduled", app.DeleteScheduledReport)
        g.PUT("/api/intel/alerts/acknowledge", app.AcknowledgeIntelAlert)
        g.PUT("/api/intel/alerts/escalate", app.EscalateIntelAlert)
