...
func (a *App) GetFlow(r *fastglue.Request) error {
        ...
        metaFlow, err := a.WhatsApp.GetFlow(ctx, &account, flow.MetaFlowID)
        if err != nil {
                a.Log.Error("Failed to get flow from Meta", "error", err, "flow_id", id, "meta_flow_id", flow.MetaFlowID)
                return r.SendErrorEnvelope(fasthttp.StatusInternalServerError, "Failed to get flow from Meta", nil, "")
        }

        // Merge Meta flow data with local flow
        response := flowToResponse(*flow)
        if metaFlow != nil {
                response.Status = metaFlow.Status
                response.PreviewURL = metaFlow.PreviewURL
                if len(metaFlow.Categories) > 0 {
                        response.Category = metaFlow.Categories[0]
                }
        }
...
func (a *App) DeleteFlow(r *fastglue.Request) error {
        ...
        ctx := context.Background()
        if err := a.WhatsApp.DeleteFlow(ctx, &account, flow.MetaFlowID); err != nil {
                a.Log.Error("Failed to delete flow from Meta", "error", err)
...
func (a *App) SaveFlowToMeta(r *fastglue.Request) error {
        ...
        if flow.MetaFlowID == "" {
                ...
                metaFlowID, err = a.WhatsApp.CreateFlow(ctx, &account, flow.Name, categories)
                if err != nil {
        ...
        if len(flow.Screens) > 0 {
                ...
                if err := a.WhatsApp.UpdateFlowJSON(ctx, &account, metaFlowID, flowJSON); err != nil {
...
func (a *App) PublishFlow(r *fastglue.Request) error {
        ...
        // Publish the flow
        if err := a.WhatsApp.PublishFlow(ctx, &account, flow.MetaFlowID); err != nil {
...
func (a *App) DeprecateFlow(r *fastglue.Request) error {
        ...
        if flow.MetaFlowID != "" {
                ...
                ctx := context.Background()
                if err := a.WhatsApp.DeprecateFlow(ctx, &account, flow.MetaFlowID); err != nil {
...
func (a *App) SyncFlows(r *fastglue.Request) error {
        ...
        metaFlows, err := a.WhatsApp.ListFlows(ctx, &account)
        if err != nil {
        ...
        for _, mf := range metaFlows {
                ...
                flowAssets, assetsErr := a.WhatsApp.GetFlowAssets(ctx, &account, mf.ID)
                if assetsErr != nil {
