...
// DownloadMediaCustom is not supported for Meta API; use DownloadMedia instead.
func (m *MetaClientAdapter) DownloadMediaCustom(ctx context.Context, account *models.WhatsAppAccount, msg *events.Message) ([]byte, error) {
        return nil, fmt.Errorf("DownloadMediaCustom is not supported by Meta Cloud API adapter")
}
...
// ListCatalogs lists all catalogs for a business via Meta API.
...
// CreateProduct adds a product to a catalog via Meta API.
...
// UpdateFlowJSON updates flow JSON via Meta API.
...
// PublishFlow publishes a draft flow via Meta API.
...
// DeprecateFlow deprecates a published flow via Meta API.
...
// GetFlow fetches a single flow from Meta API.
...
// GetFlowAssets fetches flow JSON assets from Meta API.
...
// ListFlows lists all flows for a business via Meta API.
...
// SubmitTemplate submits a template to Meta API.
...
// FetchTemplates fetches all templates from Meta API.
...
// DeleteTemplate deletes a template from Meta API.
